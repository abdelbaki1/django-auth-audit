...
def coverage(c):
    ...
    """
    c.run("coverage run --source dj-auth-audit runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/dj-auth-audit.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ dj_auth_audit")
...
def lint(c):
    ...
    """
    c.run("flake8 dj-auth-audit tests")
...
def release(c, bumpsize=''):
    ...
    import dj_auth_audit
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=dj_auth_audit.__version__))
    c.run("git push --tags")
